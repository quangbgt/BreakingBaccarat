...
class Agas:
    ...
    def __init__(self, name='Agas', commission=False, pattern='PBB', aga='P'):
        self.name = name
    ...
    def setup_bet(self):
        ...
        shoe_clean = self.res_string.replace(',', '').replace('T', '')
        if(len(shoe_clean) == 0):
            self.bet_option = 'P'
            self.bet_value = self.basebet
        else:
            if(shoe_clean.endswith(self.pattern)):
                self.bet_option = self.aga
                self.bet_value = self.basebet
    ...
    def work(self):
        ...
        while(len(self.playbox) >= self.tailoff):
            ...
            if (c1 != None and c2 != None and c3 != None and c4 != None):
                ...
                self.res_string += '{},'.format(self.winner)
